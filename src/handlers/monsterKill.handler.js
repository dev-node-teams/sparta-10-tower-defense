...
export const monsterKill = async (userId, payload) => {
  ...
  const findMonsterId = monsterMetadata.find((monster) => monster.monsterId === payload.monsterId + 1);
  ...
  let totalGold = getTotalGold(userId);
  console.log(`골드 ----> `, getTotalGold(userId));
